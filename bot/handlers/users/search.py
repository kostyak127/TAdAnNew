...
async def send_data(message: types.Message):
    ...
    if answer == '@channel' or answer == '@link':
        ...
        link = re.match('[a-zA-z0-9_]+[.][a-z]+', answer).group(0)
        ...
        if res.endswith('.csv'):
            ...
            with open(res, 'rb') as file:
                await bot.send_document(message.from_user.id, file,
                                        reply_markup=DownloadMessagesInlineKeyboard(table_name=answer,
                                                                                    table_type='link',
            ...
            await message.answer(res, reply_markup=DownloadMessagesInlineKeyboard(table_name=answer,
                                                                                  table_type='link',
                                                                                  button_text=f'Получить сообщения упоминавшие {link}').keyboard)
    else:
